...
class Simulator:
    ...
    def create_save(size,start,stop,step,device):
        cp.cuda.Device(device).use()
        list_index = cp.arange(int(start),int(stop),int(step))
        for index in list_index:    
            params = ParamsEllipses(size,device)
            dataset = DatasetImages(size,device)
            ...
            psf = DatasetPSF(size,type_psf,device)
            ...
            dirty_gauss = DatasetDirty(size,type_psf,device)
            ...
            dirty_real = DatasetDirty(size,type_psf,device)
